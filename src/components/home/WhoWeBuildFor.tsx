...
const cards = [
  ...
  },
  {
    ...
    description: (
      <>
        We build <strong className="font-extrabold text-[#059669]">learning platforms</strong>, <strong className="font-extrabold text-[#0F172A]">coaching systems</strong>, <strong className="font-extrabold text-[#0F172A]">academic automation</strong>, and <strong className="font-extrabold text-[#059669]">AI-powered education tools</strong> that help institutions <strong className="font-extrabold text-[#0F172A]">teach</strong>, <strong className="font-extrabold text-[#0F172A]">manage</strong>, and <strong className="font-extrabold text-[#059669]">grow more effectively</strong>.
      </>
    ),
    accentColor: "#059669",
    accentTile: "rgba(5,150,105,0.07)",
    accentBorder: "rgba(5,150,105,0.2)",
    accentBorderHover: "rgba(5,150,105,0.4)",
    accentTileHover: "rgba(5,150,105,0.12)",
...
function FeatureTile({
  ...
  return (
    <motion.div
      ...
      className="group/feat flex flex-col gap-2.5 p-4 sm:p-5 rounded-2xl border transition-all duration-200 cursor-default"
      style={{
        backgroundColor: card.accentTile,
        borderColor: card.accentBorder,
      }}
      onMouseEnter={(e) => {
        (e.currentTarget as HTMLDivElement).style.backgroundColor = card.accentTileHover;
        (e.currentTarget as HTMLDivElement).style.borderColor = card.accentBorderHover;
      }}
      onMouseLeave={(e) => {
        (e.currentTarget as HTMLDivElement).style.backgroundColor = card.accentTile;
    ...
    >
      ...
      <h4 className="text-[11.5px] xs:text-[12px] sm:text-[13px] font-extrabold leading-snug tracking-[-0.01em] text-[#0F172A] whitespace-nowrap overflow-hidden text-ellipsis">
        {feat.title.includes(feat.highlight) ? (
          <>
            {feat.title.split(feat.highlight)[0]}
            <span className="font-black" style={{ color: card.accentColor }}>
              {feat.highlight}
            </span>
      ...
      {/* Description */}
      <p className="text-[11.5px] sm:text-xs text-[#64748B] leading-relaxed">
        {feat.desc}
      </p>
    </motion.div>
  );
...
function AudienceCard({ card, cardIdx }: { card: (typeof cards)[0]; cardIdx: number }) {
  ...
  const descInView = useInView(descRef, { once: false, amount: 0.4 });
  ...
  return (
    <motion.div
      ...
      transition={{ duration: 0.6, delay: cardIdx * 0.15, ease: "easeOut" }}
      className="group relative rounded-3xl border border-slate-200/80 bg-white overflow-hidden shadow-sm hover:shadow-xl transition-all duration-500 hover:-translate-y-1 flex flex-col"
    >
      ...
      <motion.div
        ...
        className={`relative bg-gradient-to-br ${card.headerGradient} px-7 pt-8 pb-12 sm:px-9 sm:pt-9 sm:pb-14 overflow-hidden`}
      >
        ...
        <motion.div
          ...
          className="relative z-10 w-12 h-12 rounded-2xl bg-white/15 border border-white/25 backdrop-blur-sm flex items-center justify-center mb-4 shadow-lg"
        >
        ...
        >
          <h3 className="text-lg sm:text-2xl font-extrabold text-white tracking-[-0.02em] leading-snug">
            {card.label}
          </h3>
          <p className="text-white/70 text-xs sm:text-sm font-semibold mt-1 tracking-wide">
            {card.sublabel}
          </p>
        </motion.div>
      ...
      >
        <div className="rounded-2xl bg-white border border-slate-200/80 shadow-[0_4px_20px_rgba(0,0,0,0.06)] p-4 sm:p-5">
      ...
      >
        <div className="flex items-center gap-3 pt-4 border-t border-slate-100">
          ...
          <p className="text-[11px] sm:text-xs text-[#94A3B8] font-semibold tracking-wide">
            {card.footer}
          </p>
        </div>
      </motion.div>
...
export default function WhoWeBuildFor() {
  return (
    <section className="relative py-20 sm:py-28 lg:py-32 bg-white overflow-hidden border-t border-slate-100">
      {/* ── Dot grid background ── */}
      ...
      <div className="absolute bottom-0 left-1/3 w-[500px] h-[280px] bg-emerald-100/40 rounded-full blur-[120px] pointer-events-none" />

      <Container as="div">
        <div className="relative z-10 max-w-6xl mx-auto">

          {/* ── Section Header — each element reveals independently ── */}
          <div className="text-center max-w-2xl mx-auto mb-14 sm:mb-20">
            <Reveal delay={0} y={14}>
              <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-white border border-slate-200/90 shadow-xs text-[11px] sm:text-xs font-extrabold text-slate-500 tracking-[0.14em] uppercase mb-5">
            ...
            <Reveal delay={0.08} y={20}>
              <h2 className="text-[1.65rem] sm:text-[2.5rem] lg:text-[3rem] font-extrabold text-[#0F172A] tracking-[-0.035em] leading-[1.18]">
                Technology for organizations that want to{" "}
            ...
            <Reveal delay={0.16} y={16}>
              <p className="mt-4 text-sm sm:text-[15px] text-[#64748B] leading-relaxed">
                From early-stage startups to established universities, we engineer digital infrastructure and intelligent systems built for your domain.
              </p>
            </Reveal>
          </div>
